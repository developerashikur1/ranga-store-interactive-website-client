...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${product.image}></img>
      ...
      <p>Reviews: <b>${product.rating.rate}</b></p>
      <p>Based on <b>${product.rating.count}</b> Reviews</p>
      <h2>Price: $ <span id="cost">${product.price}</span></h2>
      <div id="btns" class=""> 
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-warning text-black">add to cart</button>
      <button id="details-btn" onclick=(detailsProduct("${product.id}")) class="btn btn-success text-white" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Details</button>
      </div>
      </div>
      `;
...
const odd = () =>{
  const display = document.getElementById('showDetails');
  display.innerHTML='';
};


// details product
const detailsProduct = (ok) => { 
  const data = allData();
  const ray = data.find(element => element.id === parseInt(ok));  
  const display = document.getElementById('showDetails');
  const div = document.createElement('div');
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="staticBackdropLabel">${ray.title}</h5>
            
          </div>
          <div class="modal-body" class="">
          <div style="text-align:center;">
          <img class="product-image w-50" style="height: 100%; text-align:center;" src=${ray.image}></img>
          </div>
          <div class="d-flex justify-content-between">
          <p>Category: ${ray.category}</p>
          <h5>Product Id: <b>${ray.id}</b></h5>
          </div>
          <p>Reviews: <b>${ray.rating.rate}</b></p>
          <p>Based on <b>${ray.rating.count}</b> Reviews</p>
          <h4>Price: $ <span id="cost">${ray.price}</span></h4>
            
            <p>details: ${ray.description.slice(0,150)}</p>
          </div>
          <div class="modal-footer d-flex justify-content-between">
            <button type="button" class="btn btn-success" data-bs-dismiss="modal" onclick="odd()">Close</button>
            <button type="button" class="btn btn-warning" onclick=(detailsBtn(${ray.id},${ray.price}))>add to cart</button>
          </div>
        </div>
      </div>
    </div>
...
// product add from details
const detailsBtn = (id, price) =>{
  count = count + 1;
  updatePrice("price", price);
  updateTaxAndCharge();
  document.getElementById("total-Products").innerText = count;
